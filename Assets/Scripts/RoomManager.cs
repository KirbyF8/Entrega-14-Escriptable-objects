...
public class RoomManager : MonoBehaviour
{
    ...
    public bool bigYellowButtonPressed = false;
    ...
    private void SetItemsInRoom()
    {
        ...
        foreach (ItemSO item in currentRoom.items)
        {
            ...
            foreach (Interaction interaction in item.interactions)
            {
                ...
                if (interaction.inputAction.keyWord.Equals("examine") || interaction.inputAction.keyWord.Equals("examinar"))
                {
                ...
                else if (interaction.inputAction.keyWord.Equals("take") || interaction.inputAction.keyWord.Equals("coger"))
                {
                    // We set up takeDictionary
                    takeDictionary.Add(item.itemName, interaction.responseDescription);
                }
                // We check if the item can be used
                else if (interaction.inputAction.keyWord.Equals("use") || interaction.inputAction.keyWord.Equals("usar"))
                {
    ...
    public void TryToChangeRoom(string direction)
    {
        ...
        {
            GameManager.Instance.UpdateLogList($"No puedes ir al {direction}");
        }
    }
    ...
    public void TryToUseItem(string itemToUse)
    {
        if (Inventory.Instance.IsItemInInventory(itemToUse))
        {
            if (useDictionary.TryGetValue(itemToUse, out ActionResponseSO actionResponse))
            {
                ...
                if (!actionResult)
                {
                    GameManager.Instance.UpdateLogList("No ocurre nada...");
                }
            }
            else
            {
        ...
        {
            GameManager.Instance.UpdateLogList($"No tienes {itemToUse} en tu inventario");
        }
    }

    public void TryToPress()
    {
        if (currentRoom.roomName == "BYB Room" && bigYellowButtonPressed == false)
        {
            GameManager.Instance.UpdateLogList($"Saltas encima del boton, pulsandolo, es lo mejor que te ha pasado desde que caiste aquí");
        }
        else if (bigYellowButtonPressed == true)
        {
            GameManager.Instance.UpdateLogList($"El boton ya esta pulsado, que pena, te gustaria pulsarlo de nuevo");
        }
        else
        {
            GameManager.Instance.UpdateLogList($"No hay nada que pulsar en esta sala");
        }
    }
